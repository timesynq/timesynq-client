            ...
            <DialogContent className={`${isMobile ? 'w-full' : 'min-w-[750px]'} h-[770px] flex flex-col`}>
                <Tabs>
                    ...
                    <TabsContent value="accepted">
                        <AcceptedWipDialogContent />
                    </TabsContent>
                    <TabsContent value="unaccepted">
                        <UnacceptedWipDialogContent />
                    </TabsContent>
                </Tabs>
            </DialogContent>
        </Dialog>
...
const AcceptedWipDialogContent = () => {
    ...
    const {
        ...
        trySearch,
        tryGoTo
    } = useSharedWipSearch(true);

    useEffect(() => {
        const fetchInitialInvites = async(): Promise<void> => {
...
const UnacceptedWipDialogContent = () => {

    const { user } = useAuth(); 
    const isMobile = useIsMobile();

    const [nameQuery, setNameQuery] = useState<string>("");
    const [searchResultMessage, setSearchResultMessage] = useState<string>("All invites");
    const [isPageQueryOpen, setIsPageQueryOpen] = useState<boolean>(false);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if(searchResult.isSuccessful){
            if(nameQuery.length > 0)
                setSearchResultMessage(`Invite search results for "${nameQuery}"`);
            else
                setSearchResultMessage("All invites");
        }
        else
            Toasts.error(searchResult.message);
    }
            ...
            <div className="h-[500px] overflow-y-auto -mx-4 px-4 no-scrollbar">
                <ul className={`${isMobile ? 'w-full' : 'min-w-[600px]'} mt-4 space-y-2 flex flex-col items-center justify-center`}>
                    { items.length > 0 ? (
                        ...
                        items.length == 0 && <p className="text-muted-foreground">No invites found.</p>
                    )}
                </ul>
            </div>
            { items.length > 0 &&
